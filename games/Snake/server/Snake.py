...
class Snake(Game):
	...
	def __init__(self, player1, player2, **options):
		...
		self._arena = Arena(self._L, self._H, int(options.get("difficulty", 1)))

		# players positions (list of positions, firsst is the head)
		self._playerPos = [[(2, self._H//2)], [(self._L-3, self._H//2)]]

		self._arena.setPlayer(2, self._H//2, 0)
		self._arena.setPlayer(self._L-3, self._H//2, 1)
	...
	def __str__(self):
		...
		for y in range(self._H):
			...
			for x in range(self._L):
				...
				lines1.append('+-' if self._arena.getWall(x, y, NORTH) else '+ ')
				# character of the x,y box
				pl = self._arena.getPlayer(x, y)
				strPl = {None: ' ', 0: 'M', 1: 'O'}[pl]
				if pl is not None and (x, y) == self._playerPos[pl][0]:
					strPl = (Fore.GREEN if pl else Fore.RED) + strPl + Fore.RESET
				# 2nd line (with WEST wall)
				lines2.append(('|' if self._arena.getWall(x, y, WEST) else ' ') + strPl)
			# add end of the line (EAST walls)
			lines1.append('+')
			lines2.append('|' if self._arena.getWall(-1, y, EAST) else ' ')
			lines.append("".join(lines1))
			lines.append("".join(lines2))
		# end the last walls in the SOUTH
		lines.append("".join(('+-' if self._arena.getWall(x, -1, SOUTH) else '+ ') for x in range(self._L))+"+")
	...
	def updateGame(self, move):
		...
		hx, hy = self._playerPos[pl][0]
		nx = hx + Ddx[direction]
		ny = hy + Ddy[direction]
		# check if there is a wall and if the new position is free
		if self._arena.getWall(hx, hy, direction):
			return LOSING_MOVE, "The move make the snakes goes into a wall"
		if self._arena.getPlayer(nx, ny) is not None:
			return LOSING_MOVE, "The move make the snakes collides..."
		# the snake move
		self._arena.setPlayer(nx, ny, pl)
		self._playerPos[pl].insert(0, (nx, ny))
		# and it may grow or not
		if self._counter[pl]%10 != 0:
			qx, qy = self._playerPos[pl].pop()
			self._arena.setNoone(qx, qy)
		# increase the counter
		self._counter[pl] = (self._counter[pl]+1) % 10

		logger.debug("self._playerPos=%s",self._playerPos)
	...
	def getDataSize(self):
		...
		"""
		return "%d %d %d" % (self._L, self._H, len(self._arena.walls))
	...
	def getData(self):
		...
		"""
		return " ".join("%d %d %d %d" % wall for wall in self._arena.walls)
